...
class port:
  portName = "/dev/ttyACM1" #Changed by p
  baudRate =9600
...
#Funcion MANIPULA
  ...
  def _Lrc(self,linea):
    if isinstance(linea, bytes):
      ...
      # return functools.reduce(operator.xor, map(bytes, linea))
      # return functools.reduce(operator.xor, linea)
      # return functools.reduce(operator.xor, map(bytes, linea))
    # else:
    #   return functools.reduce(operator.xor, map(ord, linea))
    print('linea_Lrc')
    print(linea)
    print(type(linea))
    print('linea_Lrc')
...
class Tfhka(tf_ve_ifpython):
  def GetS1PrinterDataS1(self):
    get_trama=self._States("S1")
    self.trama=get_trama.decode("utf-8")
    print('aa')
    print(self.trama)
    print('aa')
  ...
  def GetS1PrinterData(self):
    get_trama = self._States("S1")
    print('get_trama jj13')
    print(get_trama)
    print('get_trama')
    self.trama = get_trama.decode('UTF-8')
    print('self.trama')
    print(self.trama)
    print(type(self.trama))
    print('self.trama')
    #print self.trama
